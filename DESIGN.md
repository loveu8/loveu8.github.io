...
- **Link that also updates the page:** When a timeline `.tl-link` (Navigate / Map) is clicked, do both in one tap: let the native `target="_blank"` anchor navigation proceed (opens Google Maps in a new tab) *and* run the in-page click handler to update the embedded map preview — do not call `preventDefault()` to special-case this per device width. Because the anchor already uses `target="_blank"`, the browser handles opening the new tab natively (no `window.open()` scripting involved), so there is no popup-blocker risk and no conflict with the same click also driving in-page state.
