...
> - 尚未處理：風險清單分類整理、開航促銷移出主文、章內篇幅縮減（見 QA 檔）。
...
- **情境算術（非預測）**：華航 NW26 起雪梨每日（7 班／週），若每班座位等於 2025 年平均（約 308 席；2025 年 255 班、78,603 席），全年單向約 112,000 席，較 2025 增約 33,600；星宇每週 4–5 班（297 席）增 61,776–77,220。合計單向約 174,000–190,000 席，高於 2019 年的 146,268。維持 80% 載客率需單向約 14–15 萬旅客，2025 年實際為 64,553（入）／68,070（出）。假設：華航全年 7 班／週、座位同 2025 平均、星宇全年營運；實際班表與機型配置未定。
...
3. 載客率敏感度分析，不要叫 break-even load factor。真正的 BELF 需要燃油、機組、飛機租賃、機場與航管費、地勤、餐飲、銷售、維修等成本，這些拿不到。改做 Route Revenue Scenario：297 席、每週 5 班到每日、三組票價假設、三組載客率作為 sensitivity cases，算出年收入區間。**（2026-09-21 修正）載客率建議改為 70%、82%、88%**：82% 與 88% 分別是 BITRE 台澳直飛雙向載客率的 FY24/25 與 FY25/26 實測；原先的 79% 是 TRA 圖的入境單方向數字，不再建議當 benchmark。這些只是 Taiwan–Australia direct market 的 aggregate benchmark，不代表 STARLUX 或 SYD–TPE 新增座位的預期載客率；新運力進入後整體載客率可能被稀釋（見第六章情境算術），70% 作為壓力情境。三組值是建議，尚待決定。
